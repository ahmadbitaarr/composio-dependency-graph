...
zip -r "$ZIP_FILE" . \
	...
	-x "*.pem" \
	-x "project.zip"
    -x "dep-graph-submission-preview.zip" \
    -x "APPLY.txt" \
    -x "VERIFY.txt" \
    -x "googlesuper_tools.json" \
    -x "normalized-contract-samples.json" \
    -x "phase3-contract-extract.json" \
    -x "phase3-gold-candidates.json" \
    -x "phase3-gold-paths.json" \
    -x "phase3-normalizer-contract-excerpts.txt" \
    -x "phase3-semantic-excerpts.txt" \
    -x "phase4-holdout-pool.json" \
    -x "phase4-holdout-pool-v2.json" \
    -x "phase4-step1-review.json" \
    -x "schema-samples.json"
